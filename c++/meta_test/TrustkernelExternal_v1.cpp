...
static int runcmd(const char *p, char *meta_resp, size_t meta_resp_size, bool raw = false)
{
    ...
    /*
        kpha_log("kph_shell_cmd  = %s\n",p);

        id = find_kph_cmd(p);
        r = invoke_kpha_api(id)
     */
...
static int parse_buf_and_generate_key(char *buf, int buf_size)
{
    ...
    kpha_log(" YQQQ->> kph_generate return = %d, number = %d",ret,number);
    return ret;
...
static int parse_buf_and_write_file_to_pc(char *buf, int *buf_size)
{
    ...
    kpha_log("Trustkernel: filename = %s\n", filename);

    for (it = id_buf.begin(); it != id_buf.end(); it++) {
        kpha_log("Trustkernel: id_buf.first = %s\n", it->first.c_str());
        kpha_log("Trustkernel: data = %s\n", it->second.buf);
    }
...
static int handle_kph_export_device_config(char *buf, uint32_t buf_size)
{
    ...
    kpha_log("Trustkernel: dev_name = %s\n", dev_name);
    kpha_log("Trustkernel: conf_name = %s\n", conf_name);
...
static int get_sn_chipid(char *out_buf, int *out_buf_len)
{
    ...
    char *tmp_buf = NULL;
    char *p = NULL;
    char *fake_chipid = "fake_chipid123123123123123";
    p = out_buf;
...
#define COPY_BUF(dst, sour, len)  \
    ...
    attr_length = strlen(fake_chipid);
    ...
    COPY_BUF(p, fake_chipid, attr_length);
    free(tmp_buf);
    *out_buf_len = used;
    return 0;
}
...
static int generic_execute(char *inbuf, int *len)
{
    ...
    if (strncmp(inbuf, "getprop", strlen("getprop")) == 0) {
        ...
        id = find_kph_cmd(inbuf);
        r = invoke_kpha_api(id);
        kpha_log(" invoke_kpha_api = %d\n",r);
    }
